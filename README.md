...
├── frontend/                     # Static UI-transformation playground
│   ├── index.html
│   ├── css/styles.css
...
└── src/
    ...
    │   ├── TransformServer.java   # Serves frontend/ + POST /api/transform (JDK HttpServer)
...
```sh
mvn package                       # compiles, runs tests, builds target/minijava-ui.jar
java -jar target/minijava-ui.jar  # starts the server on http://localhost:8080/
```
...
# Run the server (defaults: port 8080, ./frontend)
java -cp target/classes com.minijava.ui.Main
# Optional: java -cp target/classes com.minijava.ui.Main 9000 frontend
```
...
`frontend/index.html` directly in a browser; the "Use Java backend" option simply requires
